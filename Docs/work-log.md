...
- 2026-05-01 の追加切り分けとして、種まき投稿の X API 403 が URL 付き本文起因か確認するため、`PublishSelectedPost` の投稿本文URL付与を `viral`（収穫）だけに限定した。これにより `awareness / overtime / before_after / double_question / light_achievement / cta / constraint / current_affairs` は tracking shortId と click tracking link を作らず、本文だけで X に投稿する
- `pnpm --filter @ph4k/workers typecheck` と `pnpm -r build` 成功後、`npx cdk@2.1118.2 deploy Ph4kSnsApplicationStack --require-approval never` を実行し、`2026-05-01 13:33 JST` 頃に `PublishSelectedPostHandler` を含む `Ph4kSnsApplicationStack` が `UPDATE_COMPLETE` まで到達した
- LINE に返る種まき候補にもURLが残っていたため、`OpenAiCandidateGenerator` の種まき用 prompt から「最後にLPリンクを付ける」必須ルールを削除し、URL禁止へ変更した。あわせて `awareness / overtime / before_after / double_question / light_achievement / cta / constraint / current_affairs` の候補保存前に hook/body から URL を除去し、`viral`（収穫）だけ従来どおり LP リンク付き候補を作るようにした
- `pnpm --filter @ph4k/adapters typecheck` と `pnpm -r build` 成功後、`npx cdk@2.1118.2 deploy Ph4kSnsApplicationStack --require-approval never` を実行し、`2026-05-01 13:39 JST` 頃に `GenerateCandidatesHandler` を含む `Ph4kSnsApplicationStack` が `UPDATE_COMPLETE` まで到達した
- 2026-05-01 13:40 JST の投稿失敗を追加確認し、`post-publish-bb0e00c8-65b0-4469-ba1a-1d0f219e8cab-1777610457994` は `PublishSelectedPost` で X API 403 (`You are not permitted to perform this action.`) により失敗していた。送信直前ログでは `shortId=""`、`trackingUrl=""`、`publishText` に URL なしで、URL除去後も長めの種まき本文が X 側に拒否された形だった
- 直近成功の 2026-04-30 21:26 JST 投稿は `current_affairs` の1文投稿だったため、種まき系も X へは `hook` のみを投稿する方針に変更した。LINE の投稿確認画面も `viral` 以外は実投稿と同じ `hook` のみを表示し、生成 prompt にも「hook単体で意味が通じる短い1文」を明記した
- `PublishSelectedPost` 失敗時に candidate が `pending` のまま残っていたため、失敗時は candidate を `closed` に戻し、LINE_USER_ID 宛てに失敗通知を push してから Step Functions は失敗扱いのまま返すよう修正した。13:40 に失敗した `bb0e00c8-65b0-4469-ba1a-1d0f219e8cab` も手動で `closed` に戻した
- `pnpm --filter @ph4k/workers typecheck`、`pnpm --filter @ph4k/api typecheck`、`pnpm --filter @ph4k/adapters typecheck`、`pnpm -r build` 成功後、`npx cdk@2.1118.2 deploy Ph4kSnsApplicationStack --require-approval never` を実行し、`2026-05-01 13:47 JST` 頃に `IdeasApiHandler / GenerateCandidatesHandler / PublishSelectedPostHandler` を含む `Ph4kSnsApplicationStack` が `UPDATE_COMPLETE` まで到達した
- 追加方針として「種まきと収穫は3センテンス」に揃えることにし、`OpenAiCandidateGenerator` の sales / viral prompt に `hook + body` 合計3センテンス、hookは1文目、bodyは2〜3文目、各文40字以内目安を追加した。保存前にも `normalizeThreeSentenceDraft` で3文へ正規化し、種まきはURLなし、収穫はbody末尾にLPリンク付きへ統一した
- あわせて `PublishSelectedPost` は `viral` だけ tracking URL を末尾追加しつつ、種まきも収穫も保存済みの `hook + body` をそのまま投稿する形へ戻した。LINE投稿確認画面も同じ `hook + body` を表示するため、確認内容と実投稿が一致する
- `pnpm --filter @ph4k/adapters typecheck`、`pnpm --filter @ph4k/workers typecheck`、`pnpm --filter @ph4k/api typecheck`、`pnpm -r build` 成功後、`npx cdk@2.1118.2 deploy Ph4kSnsApplicationStack --require-approval never` を実行し、`2026-05-01 13:55 JST` 頃に `GenerateCandidatesHandler / PublishSelectedPostHandler / IdeasApiHandler` を含む `Ph4kSnsApplicationStack` が `UPDATE_COMPLETE` まで到達した
